...
class MySQL
{
    ...
    private function Connect()
    {
        try {
            ...
            $this->CloseConnection();
            $this->databaseLink = new mysqli($this->hostname, $this->username, $this->password, $this->database, $this->port);

            if ($this->databaseLink->connect_error) {
                die('Connect Error (' . $this->databaseLink->connect_errno . ') '
    ...
    {
        switch ($type) {
            case 'none': // useless do not reaffect just do nothing
                //$data = $data;
                break;
            case 'str':
            case 'string':
